...
export function PortfolioCard({ portfolio }: { portfolio: Portfolio }) {
  ...
  return (
    <div className="relative group rounded-2xl bg-white border border-[#E5E7EB] hover:border-[#9CA3AF] shadow-2xs hover:shadow-lg transition-all duration-300 card-hover-effect overflow-hidden flex flex-col">
      {/* Top Right Bookmark Toggle */}
      <button
        type="button"
        onClick={toggleBookmark}
        aria-label={isBookmarked ? "Remove bookmark" : "Bookmark portfolio"}
        className="absolute top-3 right-3 z-20 w-8 h-8 rounded-full bg-white/90 backdrop-blur-xs hover:bg-white shadow-xs border border-[#E5E7EB] flex items-center justify-center text-[#4B5563] hover:text-black transition-all hover:scale-105 cursor-pointer"
      >
        <Bookmark
          size={14}
      ...
      <a
        ...
        title={`Visit ${portfolio.title} live portfolio`}
      >
        ...
        <div className="p-4 flex flex-col justify-between">
          <div className="flex items-center justify-between gap-2">
            <h3 className="font-bold text-sm sm:text-base text-[#111827] group-hover:text-black leading-tight flex items-center gap-1.5 truncate">
              <span className="truncate">{portfolio.title || portfolio.owner.displayName}</span>
            ...
            {portfolio.styleCategory && (
              <span className="text-[11px] font-medium text-[#4B5563] bg-[#F3F4F6] px-2.5 py-0.5 rounded-full shrink-0">
                {portfolio.styleCategory}
              </span>
            )}
          </div>
